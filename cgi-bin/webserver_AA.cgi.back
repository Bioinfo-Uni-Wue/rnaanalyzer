...
#         print "<div class='box h3'>";
# 		print "<br><h3>Folding:</h3></br>";
...
sub createfoldingpicture {
	...
	if ($SEQUENCELENGTH <= $MAXFOLDINGLEN && $SEQUENCELENGTH > $MAXFORNALENGTH) {
        ...
        # Output
        print "<h3>Folding (Interactive View)</h3>\n";
        print "$svg_content\n";
...
sub CPC2 {
	...
	my $RUN_CPC="python3 $CPC/CPC2.py --ORF -i $cpc_input -o $cpc_output";
	...
	while (my $line = <$fh_cpc2>) {
		...
		my ($id, $transcript_length, $peptide_length, $fickett_score, $pI, $orf_integrity, $orf_start, $coding_probability, $label) = @columns[0..8];
    ...
    print "DEBUG: ORF start = $orf_start, Peptide length = $peptide_length\n";
    if ($orf_start > 0 && $peptide_length > 0) {
    ...
    # Store or pass @$utr_coords_ref and @$utrprintout_ref to your rendering logic
    } else {
        print "CPC2 did not return a valid ORF. Skipping UTR visualization.\n";
...
sub AUGUSTUS {
    ...
    open(my $fh, '>', $input_dna) or print "Cannot write the input file for augustus: $!";
    print $fh ">$job\n$dna_sequence\n";
    close($fh);
    ...
    if ($found_exon) {
        ...
        # Optional fallback if 3′ UTR failed
        if (!@$utr3_ref) {
            print "<i>No 3′ UTR predicted. Attempting to infer from polyA signals...</i><br>";
    ...
    return ($cds_start, $cds_end, $strand, $tss_pos, $tts_pos, $found_exon); # Return both exon coords and predicted protein
}
...
sub scan_rbs {
    ...
    print "checking cds start: $cds start"


    open my $coord_fh, '>', "$TEMPDIR/$job.rbs.coords" or die $!;
    print $coord_fh "gene1\t$cds_start\t$cds_end\n";  # Repeat for all genes
    close $coord_fh;

    # 3. Set window size
    my $utr5_len = $cds_start - 1;
    $utr5_len = 20 if $utr5_len < 20;  # safety fallback

    # 4. Run RBS Finder
    my $cmd = "perl $RBSFINDER/rbs_finder.pl $seq_file $TEMPDIR/$job.rbs.coords $TEMPDIR/$job.rbs.out $utr5_len AGGAGG";
    system($cmd) == 0 or die "RBS finder failed: $?";

    # 5. Parse output
    open my $rbs_out, '<', "$TEMPDIR/$job.rbs.out" or die $!;
    while (my $line = <$rbs_out>) {
        chomp $line;
        # parse and display RBS hits
    }
    close $rbs_out;
