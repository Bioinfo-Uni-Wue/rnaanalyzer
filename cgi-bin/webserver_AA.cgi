...
# Debug: Print all parameters received (from Joahnnes delete later)
print "<pre>";
foreach my $param ($rv->param) {
    my $value = $rv->param($param);
    print "$param: $value\n";
}
print "</pre>";
...
sub drawcoloredsequence {
    ...
    system("$VIENNARNAFOLDDIR/RNAfold --infile=$seq_file | $VIENNARNAFOLDDIR/RNAplot -o svg --filename-full"); #clicable image so it opens in new tab
	...
	print "<br><img src= '$svg_file' width='650' height='650' alt='RNA Structure'</br>";
	print "\n<b>Download As: </b>";
	print "<a href='$svg_file' target='_blank'><button>SVG File</button></a>";
	print "<a href='$ps_file' target='_blank'><button>PS File</button></a>\n";
...
# 	`cp /mnt/c/Users/ama55id/Nextcloud/RNA_analyzer/rnaanalyzer/htdocs/indexandom.html /mnt/c/Users/ama55id/Nextcloud/RNA_analyzer/rnaanalyzer/tmp/$andomtemp`;
# 	#`cp /var/www/html/RNAAnalyzer/html/andomstart.html /var/www/html/RNAAnalyzer/results/andomstart$job.html`;
	
# 	`perl -p -i -e 's/REMOVEMEPLEASE/$predprotforAnDom/g' /mnt/c/Users/ama55id/Nextcloud/RNA_analyzer/rnaanalyzer/tmp/$andomtemp`;
...
sub RNAMOTIF {
	...
	open my $fh_tbl, '<', $tblout_file or die "Cannot open tblout file: $!";
...
sub CPC2 {
	...
	open(my $fh_cpc2, "<", "$cpc_output.txt") or die "Cannot open $cpc_output: $!";
	my @results;
	my $found = 0;
	...
	if ($found) {
		...
		print "No valid data found in CPC2 output\n";
	}
