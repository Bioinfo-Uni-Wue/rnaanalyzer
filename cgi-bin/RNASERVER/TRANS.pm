package RNASERVER::TRANS;

use strict;
use lib "/usr/local/lib/x86_64-linux-gnu/perl/5.30.0";
...
sub celegans {
    ...
    my $gensqstore;my @gen;my $nt;my $genlen;my $idflag;my $Oldmotifsum;my $leader;my $seqscan1;my $motifsum;my $a1;my $a2;my $crit1;my $crit2;my $crit3;my $seqscan2; my $gguakorrektpresent;
                ...
                if ((($gen[$seqscan2] eq 'u') ||($gen[$seqscan2] eq 'c'))  && (($gen[$seqscan2-1] eq 'u') || ($gen[$seqscan2-1] eq 'c'))) { #die sind die Basen die sich mit dem GG paaren sollen!!!!
                    ...
                    for ($test=0;$test<=$lengthstem1;$test++){
                        if ($stem1stringarray[$test] eq 'G' && $stem1stringarray[$test+1] eq 'G' && $stem1stringarray[$test+2] eq 'U' && $stem1stringarray[$test+3] eq 'A'){
                            if ($stem1structurearray[$test] eq ')' && $stem1structurearray[$test+1] eq ')' && $stem1structurearray[$test-1] eq '.' && $stem1structurearray[$test-2] eq '.' && $stem1structurearray[$test-3] eq '.') {
                    ...
                    $loop1nt=0;
                    if ($matchc>=3 && $gguakorrektpresent==1) {    #wenn also ein gepaarter Stem vorliegt		    
                    #Dann suchen wir den loop davor und schauen ob ja auch brav mind 3 U vorkommen !!!
                        ...
                        if ($ucount>2 && ($seqscan1-$seqscan2-$acount-$ucount<7)) {
                            $Gremark=$seqscan1;
                            $trans1=$seqscan1+4;
                                ...
                                if ($gen[$seqscan6]=~/a|g/) {
                                    if ($gen[$seqscan6+1]=~/a|g/) {       #u exeptionally allowed
                                        ...
                                        if ((($seqscan7-$seqscan6)>5)){ #| $smsitepresent==1) {
                                            if (($gen[$seqscan7]=~/a|g/) && ($gen[$seqscan7+1]=~/a|g/)||($gen[$seqscan7+1] eq 'c' && $gen[$seqscan7+2] eq 'a')) {
                   ...
                   ################# Ab hier Suchroutine weg !!! ##################################################
						...
						##########################################################################
						    ...
						    $sumenergy2=0;
						    for ($seqscan12=$seqscan7+2;$seqscan12<=$seqscan7+7;$seqscan12++) {
							for ($seqscan13=$seqscan7+10;$seqscan13<=$seqscan7+50;$seqscan13++) {
							    $energy2=0;
							    $strechC=$seqscan12;
							    $strechD=$seqscan13;
							    while (($strechD-$strechC)/2>1) {
								if ($gen[$strechC] eq 'a') {if ($gen[$strechD] eq 'u') {$energy2=$energy2+2;}else {$energy2=$energy2-2;}}
								if ($gen[$strechC] eq 'c') {if ($gen[$strechD] eq 'g') {$energy2=$energy2+3;}else {$energy2=$energy2-2;}}
								if ($gen[$strechC] eq 'g') {if ($gen[$strechD] eq 'c') {$energy2=$energy2+3;} else {
								if ($gen[$strechD] eq 'u') {$energy2++;}else {$energy2=$energy2-2;}}}
								if ($gen[$strechC] eq 'u') {if ($gen[$strechD] eq 'a') {$energy2=$energy2+2;} else {
								    if ($gen[$strechD] eq 'g') {$energy2++;}else {$energy2=$energy2-2;}}}
								$strechC++;
								$strechD=$strechD-1;
							   }
							   if ($energy2>$sumenergy2 && $energy2>(($seqscan13-$seqscan12)/2)) {
								$sumenergy2=$energy2;
								$point12=$seqscan12;
								$point13=$seqscan13;
							    }
							...
							####################################################################
							if ($sumenergy2>5 && $klammer3>=2) {
							    $leader=0;
							    if ($trans2-30 > 0) {
								for ($seqscan14=$trans2-30;$seqscan14<=$trans2-6;$seqscan14++) {
								    if (($gen[$seqscan14] eq 'a') && ($gen[$seqscan14+1] eq 'a') && ($gen[$seqscan14+2]='c') && ($gen[$seqscan14+3] eq 'u') && ($gen[$seqscan14+4] eq 'a') && ($gen[$seqscan14+5] eq 'a')) {
									$leader=1;   #Keine Ahnung ob Perl diese Terminologie versteht!! Wenn nicht aendern in 0 und 1
									$leaderw=$seqscan14;
								    }
								}
							    }
							    #Abfrage ob Treffer besser ist, als der vorher, wenn nicht, keine Ausgabe !!!!!
							    if ($sumenergy2>5 && $newhit==0){
								if ((!($Gremark=$OldGremark)) || $motifsum>$oldmotifsum) {
								    if ($idflag==0) {
									$idflag=1;
									
									$smsite='';
									for ($count=$seqscan6;$count<=$seqscan7+1;$count++) {
									    $smsite=$smsite.$gen[$count];
									    #print "$gen[$count]";
									}
								    }
								    $newhit=1;
								    #print "\n\nFuer den Loop wurde nt: $loop1nt a: $acount u: $ucount";
								    #print "\nKrit1: $crit1 Krit2: $crit2 Krit3: $crit3";
								    #print"\n";
								    #print "Sumenergy3=$sumenergy2 Leader=$leader ggua: $seqscan1 SS12=$seqscan12 SS13=$seqscan13\n";						       print "\ngguakorrektpresent: $gguakorrektpresent";			    
								    #print "\nStem1: $stem1string\n       $stem1structure";
							 
								    #print "\nStem2: $stem2string\n       $stem2structure  gepaarte: $gepaart2 bei einem Grenzwert: $gepaart2grenzwert mit bulgednt: $stem2bulgedntgesamt (nur falls 1 stem)";
								    ...
								    #Ausgabe von leader falls vorhanden
								    #print "leader: ";
								    $leaderseq='';
								    if ($leader) {
									#print "Leader!!!!!: ";
									for ($count=$leaderw;$count<=$trans2-1;$count++) {
									    #print "$gen[$count]";
									    $leaderseq=$leaderseq.$gen[$count];
									}
								    }
